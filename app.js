...
(function () {
  ...
  // ==================================================================
  // Harita sürükleme (drag-to-pan) — fare ve dokunmatik için ortak
  // ==================================================================
  let mapDrag = null; // { setPosition, centerMap }

  function setupDraggableMap(wrap, map, onTap) {
    let posX = 0, posY = 0;
    let dragging = false;
    ...
    function clampAxis(pos, contentSize, viewSize) {
      const min = Math.min(0, viewSize - contentSize);
      return Math.max(min, Math.min(0, pos));
    }
    ...
    function centerMap() {
      posX = clampAxis(-(map.offsetWidth - wrap.clientWidth) / 2, map.offsetWidth, wrap.clientWidth);
      posY = clampAxis(-(map.offsetHeight - wrap.clientHeight) / 2, map.offsetHeight, wrap.clientHeight);
      applyTransform();
    }
    ...
    wrap.addEventListener('pointermove', (e) => {
      ...
      posX = clampAxis(startPosX + dx, map.offsetWidth, wrap.clientWidth);
      posY = clampAxis(startPosY + dy, map.offsetHeight, wrap.clientHeight);
      applyTransform();
    });
    ...
    function endDrag(e) {
      ...
      if (wasTap && onTap && e && typeof e.clientX === 'number') {
        onTap(e.clientX, e.clientY);
      }
    }
    wrap.addEventListener('pointerup', endDrag);
    wrap.addEventListener('pointercancel', () => { dragging = false; wrap.classList.remove('grabbing'); });
    wrap.addEventListener('pointerleave', () => { dragging = false; wrap.classList.remove('grabbing'); });

    return { centerMap };
  }

  // ==================================================================
  // EKRAN 2.5: Yerleşim — adanın herhangi bir yerine dokunarak seçim
  // ==================================================================
  ...
  const REGION_POSITIONS = {
    orman: { left: 230, top: 190 },
    dag: { left: 470, top: 250 },
    nehir: { left: 610, top: 400 },
    ova: { left: 230, top: 460 },
    kale: { left: 420, top: 420 },
    tarla: { left: 560, top: 190 },
  };

  function nearestRegionId(x, y) {
    let bestId = null;
  ...
  function handleMapTap(clientX, clientY) {
    ...
    const regionId = nearestRegionId(localX, localY);
    if (!regionId) return;
  ...
  function selectRegion(regionId) {
    ...
    flag.style.left = `${pos.left}px`;
    flag.style.top = `${pos.top}px`;
    flag.textContent = '📍';
    $('#region-map').appendChild(flag);
  ...
  socket.on('placementStart', ({ regions }) => {
    ...
    $('#placement-status').textContent = 'Haritayı sürükleyerek gez, yerleşmek istediğin yere dokun.';
    showScreen('#screen-placement');

    const wrap = $('#region-map-wrap');
    const map = $('#region-map');
    if (!mapDrag) mapDrag = setupDraggableMap(wrap, map, handleMapTap);
    requestAnimationFrame(() => mapDrag.centerMap());
  });
  ...
  socket.on('placementUpdate', ({ players }) => {
    ...
    if (meEntry && meEntry.placed && oppEntry && !oppEntry.placed) {
      // Kendi seçimimiz zaten ekranda gösteriliyor, ek bir şey yapmaya gerek yok.
      return;
    }
