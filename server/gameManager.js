...
const GAME_DURATION_MS = 12 * 60 * 1000;  // Toplam oyun süresi: 12 dakika (uzun maraton)
...
const RAID_SOLDIER_THRESHOLD = 8;         // Saldırabilmek için gereken min. asker
const RAID_COOLDOWN_MS = 40000;           // Saldırı sonrası bekleme süresi (yavaş tempo)
const RAID_SOLDIER_COST = 3;              // Saldırıya çıkarken riske atılan asker sayısı
const DUEL_QUESTION_TIME_MS = 12000;      // Düello sorusu için süre (gerilimli, kısa)
const STARTING_VILLAGERS = 34;
const STARTING_SOLDIERS = 6;
...
class Room {
  constructor(code, io) {
    ...
    this.duel = null;       // Aktif düello: { id, attackerId, defenderId, ... }
    this.duelCounter = 0;
  }
  ...
  publicPlayersState() {
    ...
    for (const id of this.order) {
      ...
      out[id] = {
        ...
        inDuel: !!(this.duel && (this.duel.attackerId === id || this.duel.defenderId === id)),
        regionId: p.regionId,
  ...
  // Oyun bittikten sonra "Tekrar Oyna" — aynı odada, istatistikler sıfırlanarak lobiye döner.
  rematch(socketId) {
    if (this.status !== 'finished') return;
    if (!this.players[socketId]) return;

    clearTimeout(this.gameEndTimeout);
    if (this.duel) clearTimeout(this.duel.timer);
    this.duel = null;
    this.endsAt = null;
    ...
    for (const id of this.order) {
      const p = this.players[id];
      clearTimeout(p.questionTimer);
      p.villagers = STARTING_VILLAGERS;
      p.soldiers = STARTING_SOLDIERS;
      p.economy = 100;
      p.streak = 0;
      p.speed = 1.0;
      p.cooldownUntil = 0;
    ...
    this.status = 'lobby';
    this.log = [];
    this.addLog('🔄 Yeni bir sefer için lobiye dönüldü.', 'system');
    this.io.to(this.code).emit('returnToLobby');
    this.broadcastLobby();
  }
  ...
  requestQuestion(socketId) {
    ...
    if (this.duel && (this.duel.attackerId === socketId || this.duel.defenderId === socketId)) return; // düelloda
    const now = Date.now();
  ...
  // ---- Saldırı = Düello: her iki oyuncuya da aynı anda aynı soru sorulur ----
  triggerRaid(socketId) {
    ...
    if (p.soldiers < RAID_SOLDIER_THRESHOLD) {
      this.io.to(socketId).emit('raidDenied', { reason: `Saldırmak için en az ${RAID_SOLDIER_THRESHOLD} asker gerekli.` });
      return;
    }
    if (now < p.raidCooldownUntil) {
      this.io.to(socketId).emit('raidDenied', { reason: 'Saldırı hazır değil, biraz bekle.' });
      return;
    }
    if (this.duel) {
      this.io.to(socketId).emit('raidDenied', { reason: 'Zaten devam eden bir düello var.' });
      return;
    }
    ...
    // Her iki tarafın da bekleyen normal sorularını iptal et (düello önceliklidir)
    for (const id of [socketId, oppId]) {
      const pl = this.players[id];
      clearTimeout(pl.questionTimer);
      pl.currentQuestion = null;
    }

    const q = getRandomQuestion('any');
    this.duelCounter += 1;
    this.duel = {
      id: `duel${this.duelCounter}`,
      attackerId: socketId,
      defenderId: oppId,
      correct: q.correct,
      startedAt: now,
      timeLimit: DUEL_QUESTION_TIME_MS,
      responses: {},
      timer: null,
    };

    this.addLog(`⚔️ ${p.name}, ${opp.name}'e saldırıya geçti! İkisine de aynı soru soruldu.`, 'raid');
    ...
    this.io.to(socketId).emit('duelQuestion', { ...payloadBase, role: 'attacker', opponentName: opp.name });
    this.io.to(oppId).emit('duelQuestion', { ...payloadBase, role: 'defender', opponentName: p.name });

    this.broadcastState();

    this.duel.timer = setTimeout(() => this.resolveDuel(), DUEL_QUESTION_TIME_MS + 500);
  }

  handleDuelAnswer(socketId, duelId, answerIndex) {
    if (!this.duel || this.duel.id !== duelId) return;
    if (this.duel.responses[socketId]) return; // zaten cevapladı
    const correct = answerIndex !== null && answerIndex === this.duel.correct;
    this.duel.responses[socketId] = { correct, timeMs: Date.now() - this.duel.startedAt };
    this.io.to(socketId).emit('duelAnswerReceived', { correct, correctIndex: this.duel.correct });

    if (this.duel.responses[this.duel.attackerId] && this.duel.responses[this.duel.defenderId]) {
      clearTimeout(this.duel.timer);
      this.resolveDuel();
    }
  }
  ...
  resolveDuel() {
    if (!this.duel) return;
    const { attackerId, defenderId } = this.duel;
    const attacker = this.players[attackerId];
    const defender = this.players[defenderId];
    if (!attacker || !defender) { this.duel = null; return; }

    const aResp = this.duel.responses[attackerId] || { correct: false, timeMs: Infinity };
    const dResp = this.duel.responses[defenderId] || { correct: false, timeMs: Infinity };

    let winnerRole = null; // 'attacker' | 'defender' | null
    if (aResp.correct && dResp.correct) {
      winnerRole = aResp.timeMs <= dResp.timeMs ? 'attacker' : 'defender';
    } else if (aResp.correct && !dResp.correct) {
      winnerRole = 'attacker';
    } else if (!aResp.correct && dResp.correct) {
      winnerRole = 'defender';
    }
    ...
    if (!winnerRole) {
      this.addLog(`🤝 ${attacker.name} ve ${defender.name} ikisi de soruyu bilemedi, saldırı sonuçsuz kaldı.`, 'bad');
      this.io.to(this.code).emit('duelResult', {
        attackerName: attacker.name, defenderName: defender.name, success: false, winnerRole: null,
      });
      this.duel = null;
      this.broadcastState();
      return;
    ...
    const stealFrom = winnerRole === 'attacker' ? defender : attacker;
    const stealTo = winnerRole === 'attacker' ? attacker : defender;

    const stolenVillagers = Math.min(stealFrom.villagers, Math.floor(Math.random() * 3) + 2);
    const stolenSoldiers = Math.min(stealFrom.soldiers, Math.random() < 0.4 ? 1 : 0);
    stealFrom.villagers -= stolenVillagers;
    stealFrom.soldiers -= stolenSoldiers;
    stealTo.villagers += stolenVillagers;
    stealTo.soldiers += stolenSoldiers;

    if (winnerRole === 'attacker') {
      this.addLog(`⚔️ ${attacker.name} soruyu doğru bildi, saldırı başarılı! ${defender.name}'den ${stolenVillagers} kaynak${stolenSoldiers ? ` ile ${stolenSoldiers} asker` : ''} çaldı.`, 'raid');
    } else {
      this.addLog(`🛡️ ${defender.name} savunmada önce/doğru bildi, saldırıyı püskürtüp karşı hamle yaptı! ${attacker.name}'den ${stolenVillagers} kaynak${stolenSoldiers ? ` ile ${stolenSoldiers} asker` : ''} çaldı.`, 'raid');
    }
    ...
    this.io.to(this.code).emit('duelResult', {
      ...
      success: true,
      winnerRole,
      winnerName: winnerRole === 'attacker' ? attacker.name : defender.name,
      stolenVillagers,
      stolenSoldiers,
    });

    if (stealFrom.villagers <= 0 && stealFrom.soldiers <= 0) {
      stealFrom.alive = false;
      this.addLog(`🏳️ ${stealFrom.name}'in köyü çöktü!`, 'system');
      this.duel = null;
      this.broadcastState();
      this.endGame('elimination', stealTo.id);
      return;
    }
