...
(function () {
  ...
  // ==================================================================
  // EKRAN 2.5: Yerleşim / bölge seçimi
  // ==================================================================
  ...
  socket.on('placementStart', ({ regions }) => {
    ...
    regions.forEach((r) => {
      ...
      btn.innerHTML = `
        ...
        <span class="region-desc">${escapeHtml(r.desc)}</span>
      `;
      btn.addEventListener('click', () => {
        if (mySelectedRegion) return;
        mySelectedRegion = r.id;
        $$('.region-tile').forEach((t) => (t.disabled = true));
    ...
    $('#placement-status').textContent = 'Krallığını kuracağın bölgeyi seç.';
