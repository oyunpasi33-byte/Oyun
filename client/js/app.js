...
(function () {
  ...
  // Sürükleme ile dokunmayı ayırt etmek için basit bir eşik (yanlışlıkla seçim yapılmasın)
  function setupTapDetector(el, onTap) {
    let startX = 0, startY = 0, moved = 0, active = false;
    el.addEventListener('pointerdown', (e) => {
      active = true; moved = 0; startX = e.clientX; startY = e.clientY;
    });
    ...
    el.addEventListener('pointerup', (e) => {
      if (active && moved <= 8) onTap(e.clientX, e.clientY);
      active = false;
    });
  }
  ...
  socket.on('placementStart', ({ regions }) => {
    ...
    if (!tapDetectorAttached) {
      setupTapDetector($('#island-canvas'), handleCanvasTap);
      tapDetectorAttached = true;
    }
  ...
  function buildMarkerEl(prefix, name, pos, isMine) {
    ...
    const palaceHtml = isMine
      ? `<button id="palace-btn-me" class="palace-btn" title="Kalene tıkla, danışmanın soru sorsun">
           <svg viewBox="0 0 68 68" class="palace-ring">
             <circle cx="34" cy="34" r="30" class="ring-bg"></circle>
             <circle cx="34" cy="34" r="30" class="ring-fg" id="me-palace-ring-fg"></circle>
           </svg>
           <span class="palace-emoji">🏰</span>
           <span class="palace-label" id="me-palace-label">Kalene Tıkla</span>
         </button>`
    ...
    marker.innerHTML = `
      ...
      <div class="castle-info">
        <div class="castle-panel-head">
          <h3 class="castle-name" id="${prefix}-name">${escapeHtml(name)}</h3>
          <span class="speed-tag" id="${prefix}-speed-tag">⚡ 1.00x</span>
          <span class="streak-flame" id="${prefix}-streak" hidden>🔥 <b id="${prefix}-streak-count">0</b></span>
        </div>
        <div class="stat-row">
          <div class="stat-block">
            <div class="stat-icon">👤</div>
            <div class="stat-bar-track"><div class="stat-bar-fill villagers" id="${prefix}-villagers-bar"></div></div>
            <div class="stat-num" id="${prefix}-villagers-num">20</div>
          </div>
          <div class="stat-block">
            <div class="stat-icon">🗡️</div>
            <div class="stat-bar-track"><div class="stat-bar-fill soldiers" id="${prefix}-soldiers-bar"></div></div>
            <div class="stat-num" id="${prefix}-soldiers-num">4</div>
          </div>
          <div class="stat-block">
            <div class="stat-icon">💰</div>
            <div class="stat-num-solo" id="${prefix}-economy-num">100</div>
          </div>
        </div>
        ${isMine ? '' : '<div class="opp-status" id="opp-status">Hazır bekliyor...</div>'}
      </div>
    `;
    return marker;
  }
  ...
  function renderPlayers(players) {
    ...
    if (me) {
      ...
      raidBtn.textContent = me.raidReady ? '⚔️ Saldır' : `⚔️ Saldır (${me.soldiers}/8 asker)`;
  ...
  function renderSide(prefix, p) {
    ...
    if (!p.alive) {
      const marker = $(`#${prefix}-name`).closest('.castle-marker');
  ...
  function animateNumber(sel, value) {
    ...
    if (prev !== undefined && value < prev) {
      const panel = el.closest('.castle-marker');
      if (panel) {
