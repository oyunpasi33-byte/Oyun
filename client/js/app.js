...
(function () {
  ...
  function setupDraggableMap(wrap, map) {
    let posX = 0, posY = 0;
    let dragging = false;
    ...
    // Sürükleme bittiğinde altındaki pin'e yanlışlıkla tıklanmasını engelle
  ...
  socket.on('placementStart', ({ regions }) => {
    ...
    $('#placement-status').textContent = 'Haritayı sürükle, yerleşmek istediğin bölgeye dokun.';
    showScreen('#screen-placement');

    // Sürükleme sistemini kur (yalnızca ilk seferde) ve haritayı ortala
    const wrap = $('#region-map-wrap');
    if (!mapDrag) mapDrag = setupDraggableMap(wrap, map);
