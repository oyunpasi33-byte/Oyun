...
(function () {
  ...
  function generateIslandCanvas() {
    ...
    const cols = 96;
    const rows = Math.max(46, Math.round(cols * (wrapH / wrapW)));
    canvas.width = cols;
    canvas.height = rows;
    ...
    function distToBlob(x, y, center) {
    ...
    const COLORS = {
      ...
      mountain: '#7c766a',
      mountainDark: '#635d53',
      mountainLight: '#948d80',
      snow: '#e8e4da',
      lake: '#1f5083',
      lakeShore: '#2a5f96',
    };

    // Blok bazlı doku deseni üretir — tek piksel "cız" yerine temiz, kümeli piksel-sanat lekesi verir.
    function blockNoise(x, y, block, mod, offset) {
      const bx = Math.floor(x / block);
      const by = Math.floor(y / block);
      return (bx * 7 + by * 13 + Math.round(seed) + (offset || 0)) % mod;
    }
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        if (d > 1.0) {
          ...
          color = (blockNoise(x, y, 2, 7) === 0) ? COLORS.foam : COLORS.shallow;
        } else if (d > 0.84) {
          color = (blockNoise(x, y, 2, 5) === 0) ? COLORS.sandDark : COLORS.sand;
        } else {
          // kara parçası
          const lakeDist = distToBlob(x, y, lakeCenter);
          const mountainHit = mountainCenters.find((m) => inBlob(x, y, m));

          if (lakeDist < lakeCenter.r) {
            color = COLORS.lake;
          } else if (lakeDist < lakeCenter.r + 1.4) {
            color = COLORS.lakeShore;
          } else if (mountainHit) {
            const distRatio = distToBlob(x, y, mountainHit) / mountainHit.r;
            if (distRatio < 0.32) {
              color = COLORS.snow;
            } else if (blockNoise(x, y, 2, 3) === 0) {
              color = COLORS.mountainDark;
            } else if (blockNoise(x, y, 3, 4, 1) === 0) {
              color = COLORS.mountainLight;
            } else {
              color = COLORS.mountain;
            }
          } else if (inBlob(x, y, forestCenter)) {
            color = (blockNoise(x, y, 2, 4) === 0) ? COLORS.treeDark : COLORS.forest;
          } else {
            const n = blockNoise(x, y, 2, 6);
            color = n === 0 ? COLORS.grass2 : (n === 1 ? COLORS.grass3 : COLORS.grass);
          }
        }
        ctx.fillStyle = color;
  ...
  function buildMarkerEl(prefix, name, pos, isMine) {
    ...
    const castleImg = isMine ? 'img/castle-blue.png' : 'img/castle-red.png';
    ...
    const palaceHtml = isMine
      ? `<button id="palace-btn-me" class="palace-btn" title="Kalene tıkla, danışmanın soru sorsun">
           ...
           <img class="palace-castle-img" src="${castleImg}" alt="Kalen" />
         </button>`
      : `<div class="palace-btn palace-btn-readonly" aria-hidden="true"><img class="palace-castle-img" src="${castleImg}" alt="Rakip kalesi" /></div>`;
  ...
  // ==================================================================
  // Piksel asker sprite'ı + saldırı yürüyüş animasyonu
  // ==================================================================
  function buildSoldierEl(isMine) {
    const el = document.createElement('div');
    el.className = 'soldier-sprite';
    el.textContent = isMine ? '🔵' : '🔴';
    return el;
  }

  function animatePercent(el, fx0, fy0, fx1, fy1, duration, onDone) {
    const start = performance.now();
    function frame(now) {
      const t = Math.min(1, (now - start) / duration);
      const ease = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
      const fx = fx0 + (fx1 - fx0) * ease;
      const fy = fy0 + (fy1 - fy0) * ease;
  ...
  function spawnImpactBurst(pos, isShield) {
    const layer = $('#flag-layer');
    const burst = document.createElement('div');
    burst.className = isShield ? 'impact-burst shield' : 'impact-burst';
    burst.style.left = `${pos.fx * 100}%`;
    burst.style.top = `${pos.fy * 100}%`;
    burst.textContent = isShield ? '🛡️' : '💥';
    layer.appendChild(burst);
    setTimeout(() => burst.remove(), 700);
  }

  // Saldırı başladığında askerler kaleden çıkıp karşı kaleye doğru yürür (yolun %82'sine kadar),
  // sonuç gelince ya hedefe ulaşıp vurur, ya da geri püskürtülür.
  function spawnMarchingSoldiers(fromPos, toPos, isMine) {
    if (!fromPos || !toPos) return null;
    const layer = $('#flag-layer');
    const el = buildSoldierEl(isMine);
    el.style.left = `${fromPos.fx * 100}%`;
    el.style.top = `${fromPos.fy * 100}%`;
    layer.appendChild(el);

    const midFx = fromPos.fx + (toPos.fx - fromPos.fx) * 0.82;
    const midFy = fromPos.fy + (toPos.fy - fromPos.fy) * 0.82;

    const handle = { el, fromPos, toPos, midFx, midFy, done: false };

    el.classList.add('marching');
    animatePercent(el, fromPos.fx, fromPos.fy, midFx, midFy, 2200, () => {
      if (!handle.done) el.classList.add('idle-bob');
    });

    return handle;
  }

  function resolveMarch(handle, outcome) {
    if (!handle || handle.done) return;
    handle.done = true;
    const canvas = handle.el;
    canvas.classList.remove('idle-bob', 'marching');

    if (outcome === 'hit') {
      animatePercent(canvas, handle.midFx, handle.midFy, handle.toPos.fx, handle.toPos.fy, 500, () => {
        spawnImpactBurst(handle.toPos, false);
        canvas.remove();
      });
    } else if (outcome === 'repel') {
      spawnImpactBurst({ fx: handle.midFx, fy: handle.midFy }, true);
      animatePercent(canvas, handle.midFx, handle.midFy, handle.fromPos.fx, handle.fromPos.fy, 500, () => {
        canvas.remove();
      });
    } else {
  ...
  socket.on('duelQuestion', (q) => {
    ...
    // Askerler kaleden çıksın: saldıran gerçekten kimse, onun kalesinden diğerine yürür
    const attackerIsMe = q.role === 'attacker';
    const fromPos = attackerIsMe ? state.myCastlePos : state.oppCastlePos;
    const toPos = attackerIsMe ? state.oppCastlePos : state.myCastlePos;
    if (state.activeMarch) resolveMarch(state.activeMarch, 'fade');
    state.activeMarch = spawnMarchingSoldiers(fromPos, toPos, attackerIsMe);
  ...
  socket.on('duelResult', ({ attackerName, defenderName, success, winnerRole, winnerName, stolenVillagers, stolenSoldiers }) => {
    ...
    if (state.activeMarch) {
      const outcome = !success ? 'fade' : (winnerRole === 'attacker' ? 'hit' : 'repel');
      resolveMarch(state.activeMarch, outcome);
      state.activeMarch = null;
    }
